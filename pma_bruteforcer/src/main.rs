...
fn check_arguments_option<'a>(arguments: Vec<String>, option: &str) -> String{
    for (index, argument) in arguments.iter().enumerate(){
        if argument == option && index + 1 < argument.len(){
            return arguments[index + 1].clone();
        }
    }

    error_message("[-] Error in arguments.");
    exit(1);
}

fn create_client(_is_postclient: bool) -> Client{
    let client: Client;
    let timeout_duration: Duration = Duration::from_millis(TIMEOUT);
    if _is_postclient{
...
async fn pma_attack(_url: String, _pma_username: String, _wordlist: String, _threads: usize) {    
    let _post_client: Client = create_client(true);  
    let _get_client: Client = create_client(false);     
...
async fn application() -> (){
    ...
    if !url.is_empty() && !pma_username.is_empty() && !wordlist_path.is_empty(){
        pma_attack(url, pma_username, wordlist_path, threads).await;
    }
}
